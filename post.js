...
function authorMarkup(author){
  ...
  return `<a class="article-author-compact" href="${href}" aria-label="Rreth autorit ${escapeHtml(author.name)}">
    ${authorImage?`<img src="${escapeHtml(authorImage)}" alt="" width="44" height="44" loading="lazy" decoding="async">`:`<span class="article-author-fallback" aria-hidden="true">${escapeHtml(initials)}</span>`}
